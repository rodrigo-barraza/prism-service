...
Database-backed skills stored in the `agent_skills` MongoDB collection, scoped by project × user × profile (and optionally bound to one persona). `SkillService` is the one reader and writer: `toSkill` maps both stored schemas (the panel's `content` documents and the older `prompt`/`skillId` documents from `create_skill` and the Claude config importer) onto one `Skill` type. Full CRUD via REST API (`/skills`), managed through the **SkillsPanel** tab in Prism Client's Agent page. Progressive disclosure: the system prompt carries a `<skills>` catalog — one `name: description` line per skill, in name order so the cached prefix stays stable — and the model reads a body with the `load_skill` tool when a task matches. Embedding relevance (cosine ≥ 0.3) only highlights up to three catalog names in the per-turn context message; bodies are never injected. The harness emits a `skills_injected` status event listing the highlighted names.

An imported skill keeps its whole SKILL.md folder (`SkillFolderStore`: MinIO under unguessable keys, else `PRISM_SKILL_FOLDERS_DIRECTORY`); the skill document holds `folderRef` and a manifest (path, bytes, sha256). `load_skill` lists the bundled files and `read_skill_file(skill, path)` returns one — only a manifest path, never `..` or an absolute path, re-checked against the folder's real path on disk. A bundled script is text to the agent: it runs only through the shell tool and its approvals. Two importers write skills, both with a dry-run preview (Settings → Import in the client) and both confined to a registered workspace root: the Claude config importer (`POST /claude-config-import`: CLAUDE.md, `.claude/skills` folders with YAML frontmatter incl. `allowed-tools`, `.mcp.json` servers) and the Agent Plugins 1.0 importer (`POST /plugins/import`, a zip upload or a workspace folder: `plugin.json` validated against the spec schema, skills registered as `plugin:skill`, `mcp.json` servers imported disabled with `${PLUGIN_ROOT}`/`${PLUGIN_DATA}` expanded in args, env values and cwd only). **Files**: `src/services/SkillService.ts`, `src/services/skills/` (`skillMarkdown.ts`, `SkillFolderStore.ts`, `AgentPluginImportService.ts`, `zipArchive.ts`, `workspaceFolders.ts`), `src/services/ClaudeConfigImportService.ts`, `src/routes/SkillsRoutes.ts`, `src/routes/PluginsRoutes.ts`, `src/services/tool-definitions/SkillTools.ts`, `src/services/system-prompt/SkillMemoryScorer.ts`, `prism-client/src/components/SkillsPanelComponent.tsx`, `prism-client/src/components/ImportPanelComponent.tsx`.
...
- Imported skills keep their folders (`read_skill_file`); Agent Plugins 1.0 zips or folders import as `plugin:skill` + disabled MCP servers
