...
The chat UI was `prism-client/src/components/AgentChatComponent.tsx`, a 10,034-line hub, until 26 split it (2026-09-23). It is now a ~1,100-line shell that wires hooks (`src/hooks/useChat*`, `useConversation*`) into feature components: `ChatTranscript`, `Composer`, `ApprovalsAndQuestions`, `ChatStatusBar`, `GoalAndPlanPanels`, `AdminConversationView` and the message rows under `components/MessageList/`. Put a UI change in the component or hook it belongs to, not in the shell. If you must edit the shell:
...
- **tools-service.**
  - If your task changes tools-service, boot it from its worktree: `TOOLS_SERVICE_PORT=<port2> TOOLS_SERVICE_MONGO_DB_NAME=tools_test_<slug> PRISM_SERVICE_MONGO_DB_NAME=prism_test_<slug> node "$TOOLS_WT"/src/boot.ts`. The last variable points the prism settings tools-service reads (the workspace agent secret, `allowEnvFiles`) at your test database; without it they come from production's `prism` (tools-service `isolated-prism-settings-db`, 2026-09-23).
...
- **prism-client under test.**
  ...
  - **The client's `/api/tools` rewrite is not isolated by that.** It reads the vault's `TOOLS_SERVICE_URL`, the production tools-service, for task lists, tool-call logs and raw workspace files. When you boot tools-service isolated, add `LOCAL_TOOLS_PORT=<port2>` to the stand-in vault's command.
  - Then run `VAULT_SERVICE_URL=http://127.0.0.1:<port4> "$CLIENT_WT"/node_modules/.bin/next dev "$CLIENT_WT" -p <port3>`.
  - **A chat that uses files needs a workspace agent.** The client lists only agent-served workspaces (`GET /workspaces` reads the `displayRoots` of the agents in tools-service's `/admin/config`), so with none, a new chat starts with Workspace off and no file tools, and the model burns turns looking for them. Run one from source against the isolated tools-service (its `dist` is stale and registers no display roots): `VAULT_SERVICE_URL=http://127.0.0.1:1 /home/rodrigo/development/workspace-service/node_modules/.bin/tsx /home/rodrigo/development/workspace-service/src/bin/workspace-service.ts -b ws://localhost:<port2> -w <scratch git repo> -n <slug> -p <free port>` (run it in the background; `-p` moves its health port off the 5605 a real agent uses).
