...
export const TOPOLOGY_DEFINITIONS: TopologyDefinition[] = [
  {
    ...
    abbreviation: "CoT",
    description:
      "Sub-agents execute one at a time in a linear chain. Each agent receives the accumulated output from all prior agents as context, forming a chain-of-thought pipeline. Worktree merges between steps ensure file changes propagate sequentially. If any step fails, the pipeline aborts immediately.",
    paperTitle: null,
    paperAuthors: null,
    paperYear: null,
    paperUrl: null,
  ...
  },
  {
    ...
    abbreviation: "ToT",
    description:
      "Sub-agents execute in full parallel with no inter-agent communication. The orchestrator dispatches all members simultaneously and returns all results. This is the default topology — branches never merge, and each agent works independently on the same task.",
    paperTitle: null,
    paperAuthors: null,
    paperYear: null,
    paperUrl: null,
  ...
  },
  {
    ...
    phases: ["Fan-out Candidates", "Execute Parallel", "Judge Evaluation", "Select Winner"],
    configOptions: [],
    alignment: [
    ...
    flowDescription: "[A] [B] [C] → [Judge] pick best",
  },
  {
  ...
  },
  {
    ...
    phases: ["LLM Planning", "Topological Sort", "Tier-Parallel Execution", "Synthesis Merge"],
    configOptions: [],
    alignment: [
  ...
  },
  {
    ...
    abbreviation: "LATS",
    description:
      "Iterative expand-evaluate-refine search inspired by Monte Carlo Tree Search. Uses UCB1 selection to balance exploration vs exploitation, spawns multiple branches in parallel at each depth, evaluates and scores each with an LLM judge, backpropagates values along the tree, and refines the best branch with evaluator feedback at the next depth level.",
    ...
    phases: ["UCB1 Select", "Expand Branches", "LLM Evaluate", "Backpropagate", "Refine Best"],
    configOptions: [
      {
        name: "maxDepth",
        type: "number",
        defaultValue: "3",
        description: "Maximum depth of the search tree. Each depth level expands and evaluates branches.",
      },
      {
        name: "branchFactor",
        type: "number",
        defaultValue: "3",
        description: "Number of parallel branches to expand at each depth level.",
      },
      {
    ...
    flowDescription: "[B₁ B₂ B₃] → [Eval] → [Best] → [B₁' B₂'] → [Eval] → …",
  },
];
