...
import type { EmitFunction } from "../src/services/harnesses/types.ts";
...
vi.mock("../src/config.ts", async () => {
  const actual = await vi.importActual<Record<string, unknown>>("../src/config.ts");
  return {
    ...actual,
...
vi.mock("../src/utils/CostCalculator.ts", async () => {
  const actual = await vi.importActual<Record<string, unknown>>("../src/utils/CostCalculator.ts");
  return {
    ...actual,
