...
async function testOpenAI() {
  ...
  try {
    ...
    const text = response.output?.find((outputItem: any) => outputItem.type === "message")?.content?.[0]?.text;
    console.log(`${PASS} text.verbosity=medium`);
    console.log(`   → "${text?.slice(0, 80)}"`);
    passCount++;
  } catch (error: any) {
...
async function testAnthropic() {
  ...
  // Test 1: temperature, top_k, max_tokens, stop_sequences, service_tier
  try {
    ...
    console.log(`${PASS} temperature=0.5, top_k=10, max_tokens=50, stop_sequences, service_tier=auto`);
    console.log(`   → "${text.slice(0, 80)}"`);
    passCount++;
  } catch (error: any) {
